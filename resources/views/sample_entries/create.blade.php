...
@section('content')
    <div class="container">
        ...
        <h1 class="mb-4 text-secondary text-center"
            ...
            Nuevo Ingreso de Muestra
        </h1>

        <form id="entryForm" action="{{ route('sample_entries.store') }}" method="POST" novalidate>
            @csrf
            ...
            <div class="card mb-4 shadow-sm border-0" style="border-radius: 10px;">
                ...
                <div class="card-body bg-light">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            @if (!empty($selectedCode))
                                ...
                                <input type="text" name="internal_sample_code" id="internal_sample_code"
                                    class="form-control navigable" value="{{ $selectedCode }}" readonly>
                            @else
                                <!-- Si no viene preseleccionado -->
                                <select name="internal_sample_code" id="internal_sample_code" class="form-select navigable">
                                    <option value="">-- Seleccionar código interno --</option>
                                    @foreach ($availableReceptions as $reception)
                                        <option value="{{ $reception->internal_sample_code }}"
                                            data-matrix="{{ $reception->matrix }}"
                                            data-bioassays="{{ json_encode($reception->assigned_bioassays ?? []) }}">
                                            {{ $reception->internal_sample_code }}
                                        </option>
                                    @endforeach
                                </select>
                            @endif
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            @php
                                ...
                                $selectedMatrix = $selectedCode ? ($availableReceptions->firstWhere('internal_sample_code', $selectedCode)->matrix ?? '') : '';
                            @endphp
                            <input type="text" name="sample_type" id="sample_type" 
                                class="form-control navigable bg-white" 
                                value="{{ $matrixLabels[$selectedMatrix] ?? '' }}" 
                                readonly 
                                placeholder="Se completa automáticamente">
                            <input type="hidden" name="sample_type_code" id="sample_type_code" value="{{ $selectedMatrix }}">
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="concentration-wrapper">
                                <div class="input-group">
                                    <input type="number" step="any" min="0" max="100" 
                                        ...
                                        placeholder="0 - 100"
                                        value="{{ old('sample_concentration') }}">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="parameter_reading_date" class="form-control datetimepicker navigable"
                                value="{{ old('parameter_reading_date') }}" placeholder="Seleccionar fecha">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-bold">Analista</label>
                            <input type="text" name="analyst" class="form-control navigable" value="{{ old('analyst') }}">
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="observations" class="form-control navigable" rows="3">{{ old('observations') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="card mb-4 shadow-sm border-0" style="border-radius: 10px;">
                <div class="card-header bg-info text-white"
                    style="font-size: 1.5rem; border-top-left-radius: 10px; border-top-right-radius: 10px;">
                    Bioensayos Por Realizar
                </div>
                <div class="card-body bg-light">
                    <div class="alert alert-info mb-3" role="alert">
                        <i class="fas fa-info-circle"></i> La muestra debe ser ingresada para completar los bioensayos.
                    </div>
                    ...
                    <div class="row" id="bioassaysContainer">
                        ...
                        @foreach ($allBioassays as $index => $bio)
                            <div class="col-md-6 mb-2">
                                <div class="bioassay-item d-flex align-items-center {{ in_array($bio, $assignedBioassays ?? []) ? 'bioassay-assigned' : 'bioassay-disabled' }}"
                                    data-bioassay="{{ $bio }}">
                                    ...
                                    <label class="form-check-label ms-2 mb-0 flex-grow-1" for="bio{{ $index }}">
                                        {{ $bio }}
                                    </label>
                    ...
                    <div class="d-flex flex-wrap align-items-center gap-4">
                        ...
                        <div style="min-width: 120px;">
                            ...
                            <input type="text" name="validity" class="form-control-plaintext text-muted"
                                value="{{ old('validity', '01.09.2023') }}" readonly>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="d-flex justify-content-end mb-4">
                ...
                <a href="{{ route('sample_entries.index') }}" class="btn btn-secondary">Cancelar</a>
            </div>
        </form>
    </div>
@endsection
...
@push('head')
    ...
    <style>
        ...
        /* ============================================
           ESTILOS PARA LOS BIOENSAYOS - CORREGIDO
           ============================================ */
        .bioassay-item {
            padding: 12px 15px;
            background: white;
...
@push('scripts')
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // ============= CAMBIO DE CÓDIGO INTERNO (SELECT) =============
            const codeSelect = document.getElementById('internal_sample_code');
            if (codeSelect && codeSelect.tagName === 'SELECT') {
                codeSelect.addEventListener('change', function() {
                    const selectedOption = this.options[this.selectedIndex];
                    
                    // Actualizar matriz/tipo de muestra
                    const matrix = selectedOption.dataset.matrix || '';
                    document.getElementById('sample_type').value = matrixLabels[matrix] || '';
                    document.getElementById('sample_type_code').value = matrix;
                    
                    // Actualizar bioensayos
                    let bioassays = [];
                    try {
                        bioassays = JSON.parse(selectedOption.dataset.bioassays || '[]');
                    } catch (e) {
                        bioassays = [];
                    }
                    
                    updateBioassaysDisplay(bioassays);
                });
            }

            // ============= FUNCIÓN PARA ACTUALIZAR BIOENSAYOS =============
            function updateBioassaysDisplay(assignedBioassays) {
                const container = document.getElementById('bioassaysContainer');
                
                allBioassays.forEach((bio, index) => {
                    const item = container.querySelector(`[data-bioassay="${bio}"]`);
                    if (item) {
                        const checkbox = item.querySelector('input[type="checkbox"]');
                        const isAssigned = assignedBioassays.includes(bio);
                        
                        // Actualizar checkbox
                        checkbox.checked = isAssigned;
                        
                        // Actualizar clases
                        item.classList.remove('bioassay-assigned', 'bioassay-disabled');
                        item.classList.add(isAssigned ? 'bioassay-assigned' : 'bioassay-disabled');
            ...
            // Inicializar valores si hay old() value
            ...
            $('#entryForm').on('submit', function(e) {
                ...
                Swal.fire({
                    ...
                    text: "Se registrará el nuevo ingreso de muestra.",
                    icon: 'question',
