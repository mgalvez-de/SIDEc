...
<div class="container">
    ...
    @if(auth()->user()->hasRole(['Manager', 'Area Manager']) && !$isochrysis_galbana->validated_at)
        <form id="validateBioassayForm" action="{{ route('isochrysis-galbana.validate', $isochrysis_galbana->id) }}" method="POST" style="display:none">
            @csrf
            @method('PATCH')
        </form>
    @endif
